...
public class PrintPHListControl {
    
    // Ô 2: Thuộc tính - Quản lý dữ liệu và đầu ra danh sách phòng học
    PrintPHListStorage storage; // Đối tượng lưu trữ danh sách phòng học
    PrintPHListOutput output;   // Đối tượng xuất danh sách phòng học
    
    // Ô 3: Phương thức
    
    // Constructor - Khởi tạo đối tượng với đối tượng lưu trữ và xuất dữ liệu mặc định
    PrintPHListControl() {
        storage = new PrintPHListStorage();
        output = new PrintPHListOutput();
    }
    
    // Constructor - Khởi tạo với đối tượng lưu trữ và xuất dữ liệu tùy chỉnh
    PrintPHListControl(PrintPHListStorage storage, PrintPHListOutput output) {
